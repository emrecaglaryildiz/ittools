...
if [ $OS = "Ubuntu" ] && [ $VER = "16.04" ]; then
   ...
   apt -qq update
   apt -q install -y libapache2-mod-php7.2 php7.2-cli php7.2-mysql php7.2-gd php7.2-json php7.2-bcmath php7.2-mbstring php7.2-opcache php7.2-curl php-apcu php-pear snmp fping mysql-server mysql-client python-mysqldb rrdtool subversion whois mtr-tiny ipmitool graphviz imagemagick apache2
   #phpenmod mcrypt
   a2dismod mpm_event
   a2enmod mpm_prefork
   a2enmod php7.2
   ...
   apt -qq update
   apt -qq install -y libapache2-mod-php7.3 php7.3-cli php7.3-mysql php7.3-gd php7.3-json php7.3-bcmath php7.3-mbstring php7.3-opcache php7.3-apcu php7.3-curl php-pear snmp fping mariadb-server mariadb-client python-mysqldb rrdtool subversion whois mtr-tiny ipmitool graphviz imagemagick apache2
   #phpenmod mcrypt
   a2dismod mpm_event
   a2enmod mpm_prefork
...
if [ $observ_ver = 1 ]; then
   ...
   svn co -q --username "$svn_user" https://svn.observium.org/svn/observium/branches/stable observium
elif [ $observ_ver = 3 ]; then
   echo -e "${GREEN} [*] Checking out Observium Pro rolling from SVN${NC}"
   echo "Your SVN username and password is found efter you login at: https://www.observium.org/subs/"
   read -p "Please enter your SVN username: " svn_user
   svn co -q --username "$svn_user" https://svn.observium.org/svn/observium/trunk observium
else
